...
// import {IUniV3Quoter} from "../../interfaces/common/IUniV3Quoter.sol";
...
// contract StrategyPassiveManagerVelodrome is StratFeeCowManagerInitializable, IStrategyConcLiq, IStrategyVelodrome {
...
//         lpToken0 = IPool(_pool).token0();
//         lpToken1 = IPool(_pool).token1();
...
//         IBeefyRewardPool(rewardPool).notifyRewardAmount(output, feeLeft, 1 days);
...
//         // Claim rewards
//         uint256 feeBefore = IERC20Upgradeable(output).balanceOf(address(this));
...
//         uint256 claimed = IERC20Upgradeable(output).balanceOf(address(this)) - feeBefore;
//         fees = fees + claimed;
...
//             uint256 out;
//             uint256 nativeBefore = IERC20Upgradeable(native).balanceOf(address(this));
//             if (output != native) {
//                 VeloSwapUtils.swap(unirouter, outputToNativePath, amountToSwap, true);
//                 out = IERC20Upgradeable(native).balanceOf(address(this)) - nativeBefore;
//             }
...
//         uint amount = 10**IERC20MetadataUpgradeable(lpToken0).decimals() / 10;
//         if (lpToken0 == native) return amount;
//         return IUniV3Quoter(quoter).quoteExactInput(lpToken0ToNativePath, amount);
//     }

//     /// @notice Returns the price of the second token in native token.
//     function lpToken1ToNativePrice() public returns (uint256) {
//         uint amount = 10**IERC20MetadataUpgradeable(lpToken1).decimals() / 10;
//         if (lpToken1 == native) return amount;
//         return IUniV3Quoter(quoter).quoteExactInput(lpToken1ToNativePath, amount);
//     }
...
//         (int56[] memory tickCuml,) = IPool(pool).observe(secondsAgo);
...
//         IERC20Upgradeable(lpToken0).safeTransfer(feeRecipient, IERC20Upgradeable(lpToken0).balanceOf(address(this)));
//         IERC20Upgradeable(lpToken1).safeTransfer(feeRecipient, IERC20Upgradeable(lpToken1).balanceOf(address(this)));
...
//         IERC20Upgradeable(output).forceApprove(unirouter, type(uint256).max);
//         IERC20Upgradeable(output).forceApprove(rewardPool, type(uint256).max);
//         IERC20Upgradeable(lpToken0).forceApprove(nftManager, type(uint256).max);
//         IERC20Upgradeable(lpToken1).forceApprove(nftManager, type(uint256).max);
//     }

//     /// @notice removes swap permisions for the tokens from the unirouter.
//     function _removeAllowances() private {
//         IERC20Upgradeable(output).forceApprove(unirouter, 0);
//         IERC20Upgradeable(output).forceApprove(rewardPool, 0);
//         IERC20Upgradeable(lpToken0).forceApprove(nftManager, 0);
//         IERC20Upgradeable(lpToken1).forceApprove(nftManager, 0);
//     }

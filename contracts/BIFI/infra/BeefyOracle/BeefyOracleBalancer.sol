...
contract BeefyOracleBalancer {
    ...
    function getPrice(bytes calldata _data) external returns (uint256 price, bool success) {
        (BatchSwapStruct[] memory swaps, address[] memory assets) = abi.decode(_data, (BatchSwapStruct[], address[]));

        IBalancerVault.BatchSwapStep[] memory swapsArray = buildSwapStructArray(
            swaps,
            IERC20MetadataUpgradeable(assets[0]).decimals()
        );

        (bool ok, bytes memory result) = address(vault).staticcall(
            abi.encodeWithSelector(0xf84d066e, swapKind, swapsArray, assets, funds)
        );
        require(ok);
        ...
        price = BeefyOracleHelper.priceFromBaseToken(msg.sender, assets[assets.length - 1], assets[0], amountOut);
